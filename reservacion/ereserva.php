...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($temporada_completa) {
        ...
        if ($tarifa) {
            ...
            // Debug info para tarifa no encontrada
    ...
    // Debug: Log de fechas para verificar el cálculo
    error_log("Calculando tarifas día por día desde $fecha_inicio hasta $fecha_fin");
    
    while ($fecha_actual < $fecha_final) {
        $fecha_str = $fecha_actual->format('Y-m-d');
        
        // CORREGIDA: La consulta ahora verifica correctamente que la fecha esté dentro de la temporada
        ...
        // Debug: Log de la temporada encontrada para cada fecha
        error_log("Fecha: $fecha_str, Temporada: " . ($temporada_dia ? $temporada_dia['nombre'] . " (ID: {$temporada_dia['id']})" : 'No encontrada'));
        ...
        if ($temporada_dia) {
            ...
            // No se encontró temporada para esta fecha
            echo json_encode([
                'success' => false, 
                'message' => "No se encontró temporada para la fecha $fecha_str",
                'debug_info' => [
                    'fecha_problema' => $fecha_str,
                    'consulta_temporada' => $query_temporada_dia
                ]
            ]);
            exit;
        }
...
$cortesia_existente = null;
$inapam_existente = null;
...
if ($id_reserva_editar > 0) {
    try {
        ...
        // Obtener datos de cortesía si existe
        $query_cortesia = "SELECT * FROM cortesias WHERE id_reserva = ?";
        $stmt_cortesia = mysqli_prepare($conn, $query_cortesia);
        mysqli_stmt_bind_param($stmt_cortesia, 'i', $id_reserva_editar);
        mysqli_stmt_execute($stmt_cortesia);
        $result_cortesia = mysqli_stmt_get_result($stmt_cortesia);
        $cortesia_existente = mysqli_fetch_assoc($result_cortesia);
        mysqli_stmt_close($stmt_cortesia);

        // Obtener datos de INAPAM si existe
        $query_inapam = "SELECT * FROM descuentos_inapam WHERE id_reserva = ?";
        $stmt_inapam = mysqli_prepare($conn, $query_inapam);
        mysqli_stmt_bind_param($stmt_inapam, 'i', $id_reserva_editar);
        mysqli_stmt_execute($stmt_inapam);
        $result_inapam = mysqli_stmt_get_result($stmt_inapam);
        $inapam_existente = mysqli_fetch_assoc($result_inapam);
...
<body>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form id="formReserva">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <select id="tipo_reserva" name="tipo_reserva" class="form-select" required>
                                        ...
                                        <option value="cortesia" <?php echo (isset($reserva['tipo_reserva']) && $reserva['tipo_reserva'] == 'cortesia') ? 'selected' : ''; ?>>Cortesía</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                            ...
                            <div id="cortesiaSection" class="cortesia-item" style="display: <?php echo (isset($cortesia_existente) && $cortesia_existente) ? 'block' : 'none'; ?>;">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <select id="cortesia_motivo" name="cortesia_motivo" class="form-select">
                                            ...
                                            <option value="Promoción especial" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Promoción especial') ? 'selected' : ''; ?>>Promoción especial</option>
                                            <option value="Cliente frecuente" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Cliente frecuente') ? 'selected' : ''; ?>>Cliente frecuente</option>
                                            <option value="Compensación por inconveniente" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Compensación por inconveniente') ? 'selected' : ''; ?>>Compensación por inconveniente</option>
                                            <option value="Evento corporativo" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Evento corporativo') ? 'selected' : ''; ?>>Evento corporativo</option>
                                            <option value="Familia/Amigos del hotel" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Familia/Amigos del hotel') ? 'selected' : ''; ?>>Familia/Amigos del hotel</option>
                                            <option value="Prensa/Medios" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Prensa/Medios') ? 'selected' : ''; ?>>Prensa/Medios</option>
                                            <option value="Otro" <?php echo (isset($cortesia_existente['motivo']) && $cortesia_existente['motivo'] === 'Otro') ? 'selected' : ''; ?>>Otro</option>
                                        </select>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" id="cortesia_autorizado_por" name="cortesia_autorizado_por" class="form-control" 
                                               value="<?php echo htmlspecialchars($cortesia_existente['autorizado_por'] ?? ''); ?>"
                                ...
                                <div class="row mt-2">
                                    <div class="col-md-12">
                                        ...
                                        <textarea id="cortesia_descripcion" name="cortesia_descripcion" class="form-control" 
                                                  rows="3" placeholder="Detalles adicionales sobre la cortesía..." maxlength="500"><?php echo htmlspecialchars($cortesia_existente['descripcion'] ?? ''); ?></textarea>
                                    </div>
                                </div>
                            ...
                            <div class="descuento-item mb-3">
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" id="inapam_activo" name="inapam_activo" value="1" <?php echo (isset($inapam_existente) && $inapam_existente) ? 'checked' : ''; ?>>
                                    <label class="form-check-label" for="inapam_activo">
                                        <strong><i class="fas fa-percentage me-2"></i>Aplicar Descuento INAPAM</strong>
                                    </label>
                                </div>
                                
                                <div id="inapamFields" style="display: <?php echo (isset($inapam_existente) && $inapam_existente) ? 'block' : 'none'; ?>;">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" id="inapam_credencial" name="inapam_credencial" class="form-control" 
                                                   value="<?php echo htmlspecialchars($inapam_existente['credencial_id'] ?? ''); ?>"
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="number" id="inapam_descuento" name="inapam_descuento" class="form-control" 
                                                       value="<?php echo htmlspecialchars($inapam_existente['monto_descuento'] ?? '50.00'); ?>" 
    ...
    <script>
        ...
        let totalOriginalReserva = <?php echo (float)($reserva['total'] ?? 0); ?>;
        let descuentoInapamOriginal = <?php echo (float)($inapam_existente['monto_descuento'] ?? 0); ?>;
        ...
        $(document).ready(function() {
            ...
            // Inicializar estilo de cortesía si ya está seleccionada
            if ($('#tipo_reserva').val() === 'cortesia') {
                $('#tipo_reserva').addClass('tipo-cortesia');
                $('#cortesiaSection').show();
                $('#cortesia_motivo').prop('required', true);
            }

            // Inicializar campos INAPAM si ya están activos
            if ($('#inapam_activo').is(':checked')) {
                $('#inapamFields').show();
                $('#inapam_credencial').prop('required', true);
            }

            // Cargar el total original desde la base de datos
            if (totalOriginalReserva > 0) {
                $('#totalOriginal').val(formatMoney(totalOriginalReserva));
            }

            // Cargar descuento INAPAM existente
            if (descuentoInapamOriginal > 0) {
                totalOriginalReserva = totalOriginalReserva + descuentoInapamOriginal; // Recalcular el original
                $('#totalOriginal').val(formatMoney(totalOriginalReserva));
            }
            ...
            $('#formReserva').on('submit', function(e) {
                ...
                // Validar que exista al menos un pago (excepto para cortesías)
                const tipoReserva = $('#tipo_reserva').val();
                if (tipoReserva !== 'cortesia' && $('.pago-item').length === 0) {
                ...
                let mensajeConfirmacion = '¿Está seguro de que desea actualizar esta reserva?';
                
                if (tipoReserva === 'cortesia') {
                    mensajeConfirmacion = '¿Confirma que desea actualizar esta reserva de CORTESÍA?';
                }
