...
$sql = "
    SELECT 
        ...
        r.status, r.created_at, r.descuento_inapam,
...
<body class="optimizado-velocidad termica-compatible">
    ...
    <div class="fila-compacta">
            <span class="pequeño"> Tipo de Reserva</span>
            <span class="pequeño negrita"><?= strtoupper($reserva['tipo_reserva']) ?></span>
        </div>
   ...
   <div class="total-destacado no-cortar">
    <?php if ((float)$reserva['descuento_inapam'] > 0): ?>
        <div style="font-size: 10pt;">Subtotal: $<?= number_format($reserva['total'] + $reserva['descuento_inapam'], 2) ?></div>
        <div style="font-size: 10pt;">Descuento INAPAM: -$<?= number_format($reserva['descuento_inapam'], 2) ?></div>
        <div style="border-top: 1px dashed #000; margin-top: 2mm; padding-top: 1mm;">
            <strong>TOTAL: $<?= number_format($reserva['total'], 2) ?></strong>
        </div>
    <?php else: ?>
        <strong>TOTAL: $<?= number_format($reserva['total'], 2) ?></strong>
    <?php endif; ?>
